...
class EnhancedDWTConfig:
    ...
    RESULTS_DIR = "final_experiment_lstm/"
    SAVE_MODELS = True
    ...
    def get_model_save_path(self):
        ...
        import os
        path = os.path.join(self.RESULTS_DIR, "model_checkpoints")
        os.makedirs(path, exist_ok=True)
        return path

    def get_workflow_save_path(self):
        """Get path for saving workflow artifacts"""
        import os
        path = os.path.join(self.RESULTS_DIR, "workflow_artifacts")
        os.makedirs(path, exist_ok=True)
        return path
    ...
    def get_predictions_save_path(self):
        ...
        import os
        path = os.path.join(self.RESULTS_DIR, "predictions")
        os.makedirs(path, exist_ok=True)
        return path
...
# ═══════════════════════════════════════════════════════════════════════════
# SECTION 8: HAWKES + LSTM HYBRID ARCHITECTURE
# ═══════════════════════════════════════════════════════════════════════════

class HawkesLSTMHybrid(nn.Module):
    """
    Hawkes + LSTM Hybrid Architecture

    Architecture:
    Input Features → Hawkes Layer → [Features + Hawkes] → LSTM → Per-Band Heads

    Improvements over vanilla LSTM:
    - Hawkes layer models event causality (one ramp triggers another)
    - LSTM learns patterns from both raw features AND Hawkes intensity
    ...
    def __init__(self, input_size, hidden_size, num_layers, dropout,
        ...
        super(HawkesLSTMHybrid, self).__init__()
        ...
        if use_hawkes:
            ...
            lstm_input_size = input_size + hawkes_dim  # Concatenate Hawkes + original
        else:
            self.hawkes_layer = None
            lstm_input_size = input_size

        # Main LSTM encoder (takes original + Hawkes features)
        self.lstm = nn.LSTM(
        ...
        # Short-horizon LSTM (optional)
        if use_short_horizon_opt:
            self.short_horizon_lstm = nn.LSTM(
                input_size=lstm_input_size,
    ...
    def forward(self, x):
        ...
        last_output_main = self.dropout(lstm_out[:, -1, :])

        if self.use_short_horizon_opt:
            short_lstm_out, _ = self.short_horizon_lstm(x_combined)
            last_output_short = self.short_dropout(short_lstm_out[:, -1, :])
        else:
...
class BandWeightedLoss(nn.Module):
    ...
    - approximation: Large, slow trends (weight = 1.5)
    - details_4: Tiny, fast fluctuations (weight = 4.0)
    - details_3: Medium ramps (weight = 2.5)
    - details_2/1: Small, fast events (weight = 0.5/0.5)
    ...
    def __init__(self, horizons, frequency_bands=None, pos_weights=None,
        ...
        # ✅ FIX: Register pos_weights as buffers with valid names (no dots!)
        # Convert float horizons to safe buffer names (e.g., 1.0 -> h1p0)
        if pos_weights is not None:
            for h in horizons:
                # Create safe buffer name: replace '.' with 'p'
                buffer_name = f'pos_weight_h{str(h).replace(".", "p")}'
                self.register_buffer(buffer_name, torch.tensor([pos_weights[h]]))
        else:
            for h in horizons:
                buffer_name = f'pos_weight_h{str(h).replace(".", "p")}'
                self.register_buffer(buffer_name, torch.tensor([1.0]))
    ...
    def forward(self, predictions, targets):
        ...
        for h in self.horizons:
            ...
            # ✅ FIX: Occurrence loss with manual pos_weight application (device-safe)
            occurs_pred = predictions[h]['occurs'].squeeze()
            occurs_target = targets[h]['occurs'].float()

            # Get pos_weight from buffer (convert horizon to safe buffer name)
            buffer_name = f'pos_weight_h{str(h).replace(".", "p")}'
            pos_weight = getattr(self, buffer_name)

            # Manual BCE with logits and pos_weight (numerically stable)
            max_val = torch.clamp(occurs_pred, min=0)
            log_weight = (pos_weight - 1) * occurs_target + 1
            occurs_loss = (1 - occurs_target) * occurs_pred + log_weight * (
                torch.log(1 + torch.exp(-torch.abs(occurs_pred))) + max_val - occurs_pred
            )
            occurs_loss = occurs_loss.mean()
...
# ═══════════════════════════════════════════════════════════════════════════
# SECTION 10: TRAINING WITH STAGED APPROACH (FOR HAWKES) - FIXED
# ═══════════════════════════════════════════════════════════════════════════
...
def train_model_staged(model, train_loader, val_loader, criterion, config, device):
    ...
    # ✅ ENSURE MODEL AND CRITERION ARE ON DEVICE
    model.to(device)
    criterion.to(device)
    ...
    for epoch in range(config.HAWKES_PRETRAIN_EPOCHS):
        ...
        for batch_X, batch_y in train_loader:
            ...
            # ✅ RECURSIVELY MOVE ALL TARGETS TO DEVICE
    ...
    all_val_losses = [stage1_losses[-1]] * len(stage1_losses) + val_losses
...
# ═══════════════════════════════════════════════════════════════════════════
# SECTION 10B: ENHANCED EVALUATION
# ═══════════════════════════════════════════════════════════════════════════
...
def evaluate_model(model, test_loader, config, device):
    ...
    with torch.no_grad():
        for batch_X, batch_y in test_loader:
            ...
            # Get predictions (already on device)
            predictions = model(batch_X)
...
class InferenceWorkflow:
    ...
    def save_complete_workflow(self, model, scaler, selected_features,
        ...
        # ✅ FIX: LSTM-specific input size extraction
        if hasattr(model, 'hawkes_layer') and model.hawkes_layer is not None:
            # Hawkes+LSTM: input_size = original features (before Hawkes adds hawkes_dim)
            # The LSTM sees: original_features + hawkes_dim
            lstm_input_size = model.lstm.input_size
            original_input_size = lstm_input_size - self.config.HAWKES_DIM
        else:
            # Pure LSTM: input_size = lstm.input_size
            original_input_size = model.lstm.input_size
        ...
        torch.save({
            ...
            'model_config': {
                'input_size': original_input_size,  # ✅ FIXED
        ...
        with open(info_path, 'w') as f:
            ...
            f.write(f"Model: {'Hawkes+LSTM' if self.config.USE_HAWKES_PROCESS else 'LSTM'}\n")  # ✅ FIXED
    ...
    def load_workflow(checkpoint_dir):
        ...
        # ✅ FIX: Reconstruct LSTM model architecture (not Transformer)
        workflow.model = HawkesLSTMHybrid(
    ...
    def _predictions_to_dataframe(self, predictions, index=None):
        ...
        for h in self.config.PREDICTION_HORIZONS_HOURS:
            # Handle nested structure from evaluate_model()
            if 'predictions' in predictions[h]:
...
# ═══════════════════════════════════════════════════════════════════════════
# SECTION 12: MAIN PIPELINE (FREQUENCY-AWARE!)
# ═══════════════════════════════════════════════════════════════════════════
...
class EnhancedDWTPipeline:
    ...
    def run_pipeline(self):
      ...
      device = self.device  # ✅ FIX 1

      # ✅ FIX 2: Initialize variables that may not be set
      imputer = None
      selector = None
      param_config = None
      ...
      for target_col in self.config.target_columns:
          ...
          param_config = model.tune_mixed_strategy(df, nominal_dict[target_col])  # ✅ FIX 3: Now stored
      ...
      event_time = time.time() - step_start
      ...
      feature_engineering_time = time.time() - step_start
      ...
      labeling_time = time.time() - step_start
      ...
      feature_selection_time = time.time() - step_start
      ...
      print(f"✓ Model created: {'Hawkes+LSTM Hybrid' if self.config.USE_HAWKES_PROCESS else 'LSTM Baseline'}")
      ...
      if self.config.USE_STAGED_TRAINING and self.config.USE_HAWKES_PROCESS:
          model, train_losses, val_losses = train_model_staged(
              model, train_loader, val_loader, criterion, self.config, device
          )
      else:
          optimizer = torch.optim.AdamW(
              model.parameters(),
          ...
          model, train_losses, val_losses = train_model(
              model, train_loader, val_loader, optimizer, criterion, self.config, device
          )
      ...
      workflow.save_complete_workflow(
          ...
          selected_features=selected_features,
          nominal_dict=nominal_dict,
          param_config=param_config,  # ✅ Now guaranteed to be defined
          imputer=imputer,  # ✅ Now guaranteed to be defined
          variance_selector=selector,  # ✅ Now guaranteed to be defined
          results=results,
          reconstruction_results=reconstruction_results,
          train_losses=train_losses,
          val_losses=val_losses
...
# ═══════════════════════════════════════════════════════════════════════════
# SECTION 14: COMMAND-LINE INTERFACE & MAIN EXECUTION
# ═══════════════════════════════════════════════════════════════════════════

def main(data_path=None, transfer_learning=False, config_overrides=None):
    """
    Main execution function for workflow3 (LSTM-RBA)
    
    Args:
        data_path: Path to input dataset (Excel file)
        transfer_learning: Whether to use transfer learning mode
        config_overrides: Optional dict to override config parameters
        
    Returns:
        dict: Results containing predictions, reconstruction, and metrics
    """
    
    print("\n" + "="*80)
    print("🚀 LSTM-RBA WORKFLOW (WORKFLOW3)")
    print("="*80)
    
    # Set default data path if not provided
    if data_path is None:
        data_path = "./input_data/Baltic_Eagle_with_ERA5_Weather.xlsx"
        print(f"⚠️  No data path provided, using default: {data_path}")
    
    print(f"📊 Dataset: {data_path}")
    print(f"🔄 Transfer Learning: {'Enabled' if transfer_learning else 'Disabled'}")
    print("="*80 + "\n")
    ...
    if transfer_learning:
        print("\n" + "="*80)
        print("🔄 TRANSFER LEARNING MODE ACTIVATED")
        print("="*80)
        print("Loading pre-trained model for fine-tuning...")
        
        # Check if pre-trained model exists
        pretrained_model_dir = "final_experiment_lstm"
        
        if os.path.exists(pretrained_model_dir):
            print(f"✓ Found pre-trained model: {pretrained_model_dir}")
            
            try:
                # Load pre-trained workflow
                pretrained_workflow = InferenceWorkflow.load_workflow(pretrained_model_dir)
                
                # Run inference + fine-tuning on new data
                print("\n📊 Running transfer learning inference...")
                predictions = pretrained_workflow.predict(
                    pd.read_excel(data_path),
                    save_predictions=True
                )
                
                print("\n✅ Transfer learning inference completed!")
                print("="*80)
                ...
                print("Falling back to full training mode...")
...
def run_inference_only(model_dir, data_path, evaluate=True):
    """
    Run inference on new data using already trained model
    
    Args:
        model_dir: Directory with trained model checkpoint
        data_path: Path to new dataset
        evaluate: Whether to compute evaluation metrics
        
    Returns:
        dict: Predictions and optional metrics
    """
    ...
    if 'time' in new_data.columns:
        ...
        new_data['time'] = new_data['DateTime']
    
    print(f"   ✅ Data loaded: {new_data.shape}\n")
    
    # Run inference
    print("3️⃣ Running inference...")
    predictions = workflow.predict(new_data, save_predictions=True)
    print("   ✅ Inference complete\n")
    
    # Apply sigmoid to convert logits → probabilities
    print("4️⃣ Processing predictions...")
    for h in workflow.config.PREDICTION_HORIZONS_HOURS:
        pred_occurs = predictions['predictions'][h]['occurs'].flatten()
        
        # Apply sigmoid
        predictions['predictions'][h]['occurs_proba'] = sigmoid(pred_occurs)
        predictions['predictions'][h]['occurs_binary'] = (
            predictions['predictions'][h]['occurs_proba'] > 0.5
        ).astype(int)
    ...
    metrics = None
    if evaluate:
        print("5️⃣ Computing metrics...")
        try:
            metrics = _compute_inference_metrics(predictions, new_data, workflow)
...
def _compute_inference_metrics(predictions, new_data, workflow):
    """Compute evaluation metrics for inference"""
    
    # Extract ground truth events
    actual_events = predictions['events']
    
    # Create labels from ground truth
    labeler = MultiHorizonLabeler(workflow.config)
    
    # Get primary target
    primary_target = workflow.config.target_columns[0] if workflow.config.target_columns else 'Power'
    
    if primary_target in actual_events:
        labels_df = labeler.create_labels(
            new_data, 
            actual_events[primary_target]
        )
    else:
        print(f"⚠️  No events found for {primary_target}, metrics will be limited")
        return None
    ...
    for h in workflow.config.PREDICTION_HORIZONS_HOURS:
        # Predictions (sigmoid already applied)
        y_pred_proba = predictions['predictions'][h]['occurs_proba']
        y_pred = predictions['predictions'][h]['occurs_binary']
        
        # Ground truth
        occurs_col = f'event_occurs_{h}h'
        
        if occurs_col not in labels_df.columns:
            print(f"⚠️  Column {occurs_col} not found in labels")
            continue
        
        y_true = labels_df[occurs_col].values[:len(y_pred)]
        ...
        print(f"  AUC:       {auc:.4f}")
        print(f"  Actual:    {100*y_true.mean():5.2f}%")
        print(f"  Predicted: {100*y_pred.mean():5.2f}%\n")
    
    # Reconstruction metrics
    if 'reconstruction' in predictions and predictions['reconstruction']:
        recon = predictions['reconstruction']
        if 'metrics' in recon:
            print("="*80)
            print("📈 RECONSTRUCTION")
            print("="*80)
            print(f"  R²:   {recon['metrics']['r2']:.4f}")
            print(f"  RMSE: {recon['metrics']['rmse']:.2f}")
            print(f"  MAE:  {recon['metrics']['mae']:.2f}")
            print("="*80 + "\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='LSTM-RBA Event Prediction Workflow (Workflow3)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Full training (default dataset)
  python workflow3.py
  
  # Full training with custom dataset
  python workflow3.py --data path/to/data.xlsx
  
  # Transfer learning
  python workflow3.py --data path/to/data.xlsx --transfer
  
  # Inference only (no training)
  python workflow3.py --inference --model final_experiment_lstm --data path/to/data.xlsx
  
  # Override config parameters
  python workflow3.py --data data.xlsx --config NUM_EPOCHS=100 BATCH_SIZE=32
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Path to input dataset (Excel file)'
    )
    
    parser.add_argument(
        '--transfer',
        action='store_true',
        help='Use transfer learning mode'
    )
    ...
    parser.add_argument(
        '--model',
        type=str,
        default='final_experiment_lstm',
        help='Model directory for inference mode'
    )
    ...
    parser.add_argument(
        '--no-evaluate',
        action='store_true',
        help='Skip evaluation metrics (inference mode only)'
    )
    ...
    if args.inference:
        ...
        # TRAINING MODE (full or transfer)
        result = main(
            data_path=args.data,
    ...
    print("\n✅ Workflow3 execution completed successfully!")